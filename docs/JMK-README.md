...
| **Syscalls** | 16 system calls via `int 0x80`: exit, fork, yield, send, recv, sleep, getpid, puts, brk, kill, getticks, open, read, write, close, list |
| **Drivers** | 8259 PIC remapping, PIT timer at 1000Hz, PS/2 keyboard with scan-code translation, ATA PIO disk |
| **Filesystem** | In-memory RAMFS with open/read/write/close/list |
| **IDT** | 256-entry interrupt descriptor table with exception handlers |
| **Shell** | Interactive command shell in the init process (PID 1) |

**Total Size**: ~10,000 lines of Jasterish across 10 source files.
...
|  (pmm_alloc / vmm_map)    |
+---------------------------+
|  Hardware Abstraction     |  <- PIC, PIT, Keyboard, Serial, ATA
|  (pic_init / pit_handler) |
+---------------------------+
|  Boot & CPU Setup         |  <- GDT, IDT, Multiboot2, stack
|  (_start / gdt_init)      |
+---------------------------+
```
...
|-- syscall.jstr     (791 lines)  System call interface: 16 syscalls via int 0x80
|-- drivers.jstr     (703 lines)  PIC, PIT timer, PS/2 keyboard drivers
|-- idt.jstr         (~500 lines) Interrupt Descriptor Table, exception handlers, IRQ dispatch
|-- disk.jstr        (~500 lines) ATA PIO disk driver (LBA28 read/write)
|-- vfs.jstr         (~500 lines) In-memory filesystem (RAMFS) with open/read/write/close
...
|-- EXPANSION_PLAN.md            Phase 2 expansion roadmap
```
...
idt.jstr     --jstar-->   idt.o
disk.jstr    --jstar-->   disk.o
vfs.jstr     --jstar-->   vfs.o
...
| 11 | `sys_open` | `name`, `mode` | Open a file |
| 12 | `sys_read` | `fd`, `buf`, `count` | Read from file descriptor |
| 13 | `sys_write` | `fd`, `buf`, `count` | Write to file descriptor |
| 14 | `sys_close` | `fd` | Close file descriptor |
| 15 | `sys_list` | (none) | List filesystem contents |

### Syscall ABI

```
RAX = syscall number (0-15)
RDI = argument 1
RSI = argument 2
RDX = argument 3
Return value in RAX
